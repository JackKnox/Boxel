...
VkResult vulkan_pipeline_layout_create(box_renderer_backend* backend, VkPipelineShaderStageCreateInfo** out_shaders, box_renderstage* out_renderstage) {
    ...
    for (u32 i = 0; i < BOX_SHADER_STAGE_TYPE_MAX; ++i) {
        shader_stage* shader_stage = &out_renderstage->stages[i];
        if (shader_stage->file_size == 0) 
            continue;

		VkPipelineShaderStageCreateInfo* stage_info = darray_push_empty(*out_shaders);
		stage_info->sType  = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
		stage_info->stage  = box_shader_type_to_vulkan_type((box_shader_stage_type)i);
		stage_info->pName  = "main";

		VkShaderModuleCreateInfo create_info = { VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO };
		create_info.pCode    = shader_stage->file_data;
		create_info.codeSize = shader_stage->file_size;
        ...
        bfree(shader_stage->file_data, shader_stage->file_size, MEMORY_TAG_CORE);
        shader_stage->file_data = 0;
        shader_stage->file_size = 0;
    }

    if (out_renderstage->descriptor_count > 0) {
        // Collect descriptor data into vulkan structs.
		VkDescriptorSetLayoutBinding* descriptor_bindings = darray_reserve(VkDescriptorSetLayoutBinding, out_renderstage->descriptor_count, MEMORY_TAG_RENDERER);
		VkDescriptorPoolSize* descriptor_pools = darray_reserve(VkDescriptorPoolSize, out_renderstage->descriptor_count, MEMORY_TAG_RENDERER);

        for (u32 i = 0; i < out_renderstage->descriptor_count; ++i) {
			VkDescriptorSetLayoutBinding* binding = darray_push_empty(descriptor_bindings);
			binding->binding = out_renderstage->descriptors[i].binding;
			binding->descriptorType = box_descriptor_type_to_vulkan_type(out_renderstage->descriptors[i].descriptor_type);
			binding->stageFlags = box_shader_type_to_vulkan_type(out_renderstage->descriptors[i].stage_type);
...
b8 vulkan_renderstage_create(
    ...
    CHECK_VKRESULT(
        vulkan_pipeline_layout_create(
            backend, 
            &shader_stages, 
            out_renderstage),
        "Failed to create internal Vulkan pipeline layout");
    
    if (out_renderstage->mode == RENDERER_MODE_GRAPHICS) {
        ...
        VkViewport viewport = {};
        viewport.x = 0.0f;
        viewport.y = (f32)bound_rendertarget->size.height;
	    ...
	    VkVertexInputAttributeDescription* attributes = darray_reserve(VkVertexInputAttributeDescription, out_renderstage->graphics.vertex_attribute_count, MEMORY_TAG_RENDERER);

        u64 attribute_stride = 0;
        for (u32 i = 0; i < out_renderstage->graphics.vertex_attribute_count; ++i) {
			box_render_format attribute = out_renderstage->graphics.vertex_attributes[i];
		...
		if (out_renderstage->graphics.vertex_attribute_count > 0) {
        ...
        input_assembly.topology = VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST; // TODO: Use graphics.topology_type for this
    ...
    else if (out_renderstage->mode == RENDERER_MODE_COMPUTE) {
        BX_ASSERT(shader_stages[0].stage == VK_SHADER_STAGE_COMPUTE_BIT && "Renderstage is compute mode but contains not compute shaders");
...
void vulkan_renderstage_bind(
    ...
    if (renderstage->mode == RENDERER_MODE_GRAPHICS) {
        ...
        if (renderstage->graphics.index_buffer != NULL) {
            internal_vulkan_renderbuffer* index_buffer = (internal_vulkan_renderbuffer*)renderstage->graphics.index_buffer->internal_data;
