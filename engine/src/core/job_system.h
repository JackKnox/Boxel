...
typedef b8(*job_worker_func)(struct job_worker* worker, void* job, void* arg);

typedef struct job_worker {
	// Running = jon thread actually open or not.
...
// Signals the worker thread to quit execution. If should_quit is TRUE, the worker will finish processing all queued jobs before exiting. If FALSE, the worker may exit immediately and discard remaining jobs.
